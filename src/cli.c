...
NRF_CLI_CDC_ACM_DEF(cli_cdc_acm_transport);
NRF_CLI_DEF(cli_cdc_acm,
            "mkass420_nrf52 > ",
            &cli_cdc_acm_transport.transport,
            '\n',
            4);
...
void cli_start(){
    nrf_cli_start(&cli_cdc_acm);
}

void cli_process(){
    nrf_cli_process(&cli_cdc_acm);
}

void cli_init(void){
    ret_code_t ret;
    ret = nrf_cli_init(&cli_cdc_acm, NULL, true, false, NRF_LOG_SEVERITY_INFO);
    APP_ERROR_CHECK(ret);
}
